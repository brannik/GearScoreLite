...
PaperDollFrame:HookScript("OnShow", MyPaperDoll)
 ...
 function GetDKP()
	PaperDollFrame:CreateFontString("GearScore21")
	GearScore21:SetFont("Fonts\\FRIZQT__.TTF", 10)
	TEMP = ""
	...
	if test ~= nil then
		...
		local PlayerIndex = GuildNameToIndex(playerName,true);
		...
		if check == nil then
			PlayerIndex = GuildNameToIndex(officernote,true);
			local name2,rank2,rankindex2,level2,level2,class2,note2,officernote2,online2,status2,classFilename2 = GetGuildRosterInfo(PlayerIndex);
			GearScore21:SetText(string.match(officernote2,"%d+") .. " DKP")
	
		else
			--GearScore21:SetText("No DKP")
			GearScore21:SetText(check .. " DKP")
		end
		--frame.button4:SetText("ALL IN [".. TEMP .."]")
	else
		GearScore21:SetText("No GUILD")
	end

	

	
	GearScore21:SetTextColor(0,1,0,1)
	GearScore21:SetPoint("BOTTOMLEFT",PaperDollFrame,"BOTTOMLEFT",35,90)
	GearScore21:Show()
end

PaperDollFrame:HookScript("OnShow", GetDKP)
